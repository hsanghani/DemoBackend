...
export async function getUserByLatLong(userId:string,lat: number,long: number){
  ...
  try {
    ...
    console.log(nearByUser,"nearByUserData");

    // return nearByUser;

    const updatedUsers = nearByUser.filter(user => user._id.toString() !== userId);

    const registrationTokens = updatedUsers.map(user => user.token);
    ...
    if (registrationTokens.length > 0) {
      const message = {
        ...
        data:  {}, // If you want to send additional data along with the notification 
      };
